...
class ControllerAI {
	...
	getMaximums() {
		let rmax = board.row.map(r => ({
			row: r, cards: r.cards.filter(c => c.isUnit()).reduce((a, c) =>
...
class Player {
	...
	async activateLeader() {
		// Don't activate if conditions aren't met (e.g. no weather for Foltest)
		if (this.leader.canActivate && !this.leader.canActivate(this.leader, this)) {
			return;
		}
	...
	enableLeader() {
		...
		if (this.id === 0 && this.leader.activated.length > 0) {
			this.elem_leader.addEventListener("click",
				async () => {
					const canUse = !this.leader.canActivate || this.leader.canActivate(this.leader, this);
					await ui.viewCard(this.leader, canUse ? async () => await this.activateLeader() : null);
				},
				false);
		} else {
			this.elem_leader.addEventListener("click", async () => await ui.viewCard(this.leader), false);
	...
	constructor(card_data, player) {
		...
		if (this.abilities.length > 0) {
			...
			for (let x of this.abilities) {
				...
				if ("canActivate" in ab) this.canActivate = ab.canActivate;
			}
		}
